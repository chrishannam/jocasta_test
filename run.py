...
import piconzero.piconzero as pz, time
...
try:
    ...
    def set_speeds(power_left, power_right):
        ...
        print("raw left: {}".format(power_left))
        print("raw right: {}".format(power_right))

        # Take the 0-100 inputs down to 0-1 and reverse them if necessary
        #power_left = (power_left * 1.28 * motor_multiplier)
        #power_right = (power_right * 1.28 * motor_multiplier)
        # Print the converted values out for debug
        print("left: {}".format(power_left))
        print("right: {}".format(power_right))
        pz.forward(power_left)
        return

        pz.setMotor(0, power_left)
        pz.setMotor(1, power_left)
